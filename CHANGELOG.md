...
- **Typing indicator durante generación de informes (RoadmapBotTelegram #13)**: nuevo context manager async `_TypingAction` que envía `ChatAction.TYPING` cada 4 s en una tarea paralela. Las llamadas al agente se mueven a `asyncio.to_thread()` para no bloquear el event loop. Aplicado a `/liga`, `/equipo`, `/jornada`, `/compare` y `/jugador`.
- **Bienvenida personalizada al primer uso (RoadmapBotTelegram #14)**: `/start` detecta si el usuario ha ejecutado el comando antes usando `_is_first_use(user_id)` (persiste en `data/first_use.json`). Los nuevos usuarios reciben un segundo mensaje con guía de primeros pasos paso a paso. Si `ALLOWED_GROUP_ID` está activo, también se informa del control de acceso.
- **Aliases adicionales en español + /goleadores + /partido (RoadmapBotTelegram #15)**: `/clasificacion` → alias de `/tabla`. Nuevo handler `cmd_goleadores` con `@_cooldown(15)` que muestra el top de goleadores usando `player_loader`; admite N opcional (máx. 25). Nuevo handler `cmd_partido` que busca un partido por su ID numérico en el CSV local y muestra resultado, fecha y jornada.
- **Logging de uso por comando (RoadmapBotTelegram #16)**: función `_log_usage(user_id, command, **kwargs)` que escribe entradas JSONL en `data/usage_log.jsonl` (timestamp UTC, `user_id` hasheado SHA-256 de 12 chars, nombre del comando y parámetros de competición). Aplicado a `/liga`, `/equipo`, `/jornada`, `/compare`, `/jugador`, `/tabla` y `/goleadores`.
- **Modo mantenimiento configurable (RoadmapBotTelegram #17)**: decorador `@_require_not_maintenance` aplicado a todos los handlers excepto `/start`. Si la variable de entorno `BOT_MAINTENANCE_MSG` está definida, el handler responde inmediatamente con `🔧 <mensaje>` sin ejecutar la lógica del comando. Permite anunciar cierres planificados sin apagar el bot.
...
- **Ocultar JSON bruto para usuarios no técnicos (RoadmapDashboard #13)**: el expander `🗂️ Datos en bruto (JSON)` ahora solo aparece si `BETA_DEBUG=true` (o `1`/`yes`) está definido en `.env`. En producción (sin esa variable) el expander es invisible para usuarios finales. Se añade `BETA_DEBUG` a `.env.example` con su descripción.
- **Compatibilidad mobile (RoadmapDashboard #14)**: se inyecta un bloque CSS vía `st.markdown(unsafe_allow_html=True)` que se activa en pantallas ≤768 px. Las columnas (`div[data-testid="column"]`) colapsan a ancho completo, las imágenes respetan el viewport (`max-width: 100%`) y el sidebar no ocupa espacio mínimo forzado. Todas las imágenes de escudos y foto de jugador pasan a `use_container_width=True` para eliminar anchos fijos (`width=60`/`width=120`) que se rompen en móvil.
- **Modo oscuro consistente en gráficos (RoadmapDashboard #15)**: nueva función `set_chart_theme(dark: bool)` en `src/visualizer.py` que aplica `plt.style.use('dark_background') + sns.set(style='darkgrid')` en modo oscuro o `plt.style.use('default') + sns.set(style='whitegrid')` en modo claro. `_detect_dark_mode()` en `app.py` lee en orden: (1) `STREAMLIT_DARK_MODE` env var; (2) `[theme] base` de `.streamlit/config.toml` si existe; (3) defecto claro. Se llama una vez al arrancar el dashboard. Se añade `STREAMLIT_DARK_MODE` a `.env.example`.
