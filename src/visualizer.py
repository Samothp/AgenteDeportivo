...
def plot_goals_distribution(df: pd.DataFrame, output_path: str) -> str:
    ...
    sns.histplot(df['goles_totales'], bins=range(0, int(df['goles_totales'].max()) + 2), kde=False, ax=ax)
...
def plot_possession_distribution(df: pd.DataFrame, output_path: str) -> str:
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
    melted = df[['posesion_local', 'posesion_visitante']].melt(var_name='Equipo', value_name='Posesión')
    fig, ax = plt.subplots(figsize=(10, 5))
...
def plot_card_statistics(df: pd.DataFrame, output_path: str) -> str:
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
    cards = pd.DataFrame({
        'Tarjetas amarillas': df['amarillas_local'].sum() + df['amarillas_visitante'].sum(),
        'Tarjetas rojas': df['rojas_local'].sum() + df['rojas_visitante'].sum(),
    }, index=[0])
